...
#ifndef FX_DATETIME_HPP
#define FX_DATETIME_HPP
...
{
public:
	FXTimeSpan(int seconds)
	{
		Init(0, 0, 0, seconds, 0);
	}

	FXTimeSpan(int hours, int minutes, int seconds)
	{
		Init(0, hours, minutes, seconds, 0);
	}

	FXTimeSpan(int days, int hours, int minutes, int seconds)
	{
		Init(days, hours, minutes, seconds, 0);
	}

	FXTimeSpan(int days, int hours, int minutes, int seconds, int milliseconds)
	{
	...
	static FXTimeSpan FromMilliseconds(int milliseconds)
	{
		return FXTimeSpan(0, 0, 0, 0, milliseconds);
	}

	static FXTimeSpan FromSeconds(int seconds)
	{
		return FXTimeSpan(0, 0, 0, seconds, 0);
	}

	static FXTimeSpan FromMinutes(int minutes)
	{
		return FXTimeSpan(0, 0, minutes, 0, 0);
	}

	static FXTimeSpan FromHours(int hours)
	{
		return FXTimeSpan(0, hours, 0, 0, 0);
	}

	static FXTimeSpan FromDays(int days)
	{
		return FXTimeSpan(days, 0, 0, 0, 0);
	}
	...
	bool operator<(const FXTimeSpan &ts) const
	{
...
public:
	...
	FXDateTime(int day, int month, int year)
	{
		Init(day, month, year, 0, 0, 0, 0);
	}

	FXDateTime(int day, int month, int year, int hour, int minute, int second)
	{
		Init(day, month, year, hour, minute, second, 0);
	}

	FXDateTime(int day, int month, int year, int hour, int minute, int second, int millisecond)
	{
		Init(day, month, year, hour, minute, second, millisecond);
	}

	FXDateTime(time_t timestamp)
	{
	...
	std::string GetNameOfDay(int day)
	{
		...
		{
			throw FXException("Day %d is not in valid weekday range ( %d - %d )", day, MIN_WEEKDAY, MAX_WEEKDAY);
		}
	}
	...
	std::string GetNameOfMonth(int month)
	{
		...
		{
			throw FXException("Month %d is not in valid range ( %d - %d )", month, MIN_MONTH, MAX_MONTH);
		}
	}

	Day GetDayOfWeek()
	{
		return static_cast<FXDateTime::Day>(GetTMStruct().tm_wday);
	}
	...
	bool operator<(const FXDateTime &dt) const
	{
	...
	FXDateTime operator+(const FXTimeSpan &ts)
	{
		FXDateTime *tmp = this;
		tmp->Add(&ts);
		return *tmp;
	...
	void operator+=(const FXTimeSpan &ts)
	{
		Add(&ts);
	}
...
protected:
	...
	void Init(int day, int month, int year, int hour, int minute, int second, int millisecond)
	{
		...
		if (!IsValidMonth(month))
		{
			throw FXException("Month %d is not in range", month);
		}

		if (!IsValidDayOfMonth(day, month))
		{
			throw FXException("Day %d is not in month %d's range", day, month);
		}

		if (!IsValidHour(hour))
		{
			throw FXException("Hour %d is not in valid range ( %d - %d )", hour, MIN_HOUR, MAX_HOUR);
		}

		if (!IsValidMinute(minute))
		{
			throw FXException("Minute %d is not in valid range ( %d - %d )", minute, MIN_MINUTE, MAX_MINUTE);
		}

		if (!IsValidSecond(second))
		{
			throw FXException("Second %d is not in valid range ( %d - %d )", second, MIN_SECOND, MAX_SECOND);
		}

		if (!IsValidMillisecond(millisecond))
		{
			throw FXException("Millisecond %d is not in valid range ( %d - %d )", millisecond, MIN_MILLISECOND, MAX_MILLISECOND);
		}
